 ...
 * SystemStatus.h
 *
 * Calculates the Gravity torque and inertia of the arm with values from all axises.
 */
...
typedef struct AtlasArmConstants
{
  ...
  AtlasArmConstants(double fcgx, double fcgy, double fcgz, double bcgx, double bcgy, double bcgz, double th1offset, double d1, double a1, double alpha1,
                    ...
                    double opPointOffsetx, double opPointOffsety, double opPointOffsetz, FeedbackDevice* axis1Angle, FeedbackDevice* axis2Angle,
                    FeedbackDevice* axis3Angle, FeedbackDevice* axis4Angle, FeedbackDevice* axis5Angle, FeedbackDevice* axis6Angle)
                    : fcgx(fcgx), fcgy(fcgy), fcgz(fcgz), bcgx(bcgx), bcgy(bcgy), bcgz(bcgz), th1offset(th1offset), d1(d1), a1(a1), alpha1(alpha1),
                      ...
                      JOINT1ANGLE(axis1Angle), JOINT2ANGLE(axis2Angle), JOINT3ANGLE(axis3Angle), JOINT4ANGLE(axis4Angle), JOINT5ANGLE(axis5Angle), JOINT6ANGLE(axis6Angle)
                    {
...
public:
    ...
    // Description: Calculates the gravity and inertia values of all axises.
    //              Uses the axis constants in the calculations, so no parameters are necessary.
    // Arguments:   None
    // Returns:     Nothing
    void update();

    // Description: Returns the calculated gravity of the desired axis.
    // Arguments:   id - the ID of the axis requesting info.
    // Returns:     the torque gravity of the given axis.
    double getGravity(uint32_t id);

    // Description: Returns the calculated inertia of the desired axis.
    // Arguments:   id - the ID of the axis requesting info.
    // Returns:     the inertia of the given axis.
